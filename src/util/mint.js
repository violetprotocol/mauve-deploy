...
const mint = async (nonFungiblePositionManager, tokens, minter, receiver, eatSigner, domain) => {
    const mintParams = {
    ...
    const { eat, expiry } = await (0, generateAccessToken_1.generateAccessTokenForMulticall)(eatSigner, domain, minter, nonFungiblePositionManager, mintMulticallParameters);
    const [txData] = await nonFungiblePositionManager
        .connect(minter)
        .callStatic["multicall(uint8,bytes32,bytes32,uint256,bytes[])"](eat.v, eat.r, eat.s, expiry, mintMulticallParameters);
    await nonFungiblePositionManager
        .connect(minter)["multicall(uint8,bytes32,bytes32,uint256,bytes[])"](eat.v, eat.r, eat.s, expiry, mintMulticallParameters);
    const { tokenId: tokenId, liquidity: liquidity, amount0: amount0, amount1: amount1, } = nonFungiblePositionManager.interface.decodeFunctionResult("mint", txData);
    const mintedResults = {
        tokenId,
        liquidity,
        amount0,
